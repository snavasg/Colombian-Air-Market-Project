...
```mermaid
graph TD
    START([📄 150 Papers + Taxonomy]) --> P1
    
    subgraph P1[Phase 1 — Context Extraction]
        direction LR
        FCI[🔍 FCI Rules<br/><i>Python</i>]
        SEM[🧠 FCI Semantic<br/><i>Haiku 3</i>]
        EMB[📐 Embeddings<br/><i>Local</i>]
        EXT[📝 Extractor<br/><i>Haiku 4.5</i>]
    end
    
    P1 --> ACC[⚖️ Accuracy Judge<br/><i>Haiku 3 × 3 parallel votes</i><br/>summary + abstract + JEL + spatial]
    
    ACC --> QG{🚦 Quality Gate}
    QG -->|PASS| TAX
    QG -->|FAIL| REJ[🔄 ReJudge<br/><i>Haiku 3 × 5 votes</i>]
    REJ --> TAX
    
    TAX[🏷️ Taxonomy Matcher<br/><i>Embeddings + Haiku 4.5 selector</i>] --> ENR[➕ Enrichment<br/><i>Haiku 3</i>]
    
    ENR --> CRIT{Disputes?}
    CRIT -->|Yes| CR[👨‍⚖️ Critic<br/><i>Haiku 4.5</i>]
    CRIT -->|No| XC
    CR --> XC[🔀 Cross-Check<br/><i>Python</i>]
    
    XC --> REP[📋 Reporter<br/><i>Haiku 4.5</i>]
    REP --> OUT[📊 Merge + Charts + JSON]
    OUT --> FIN([✅ audit_consolidated.json])

    style P1 fill:#f0f4ff,stroke:#0075C1
